...
const formatDiffPercent = (basisPoints: number): string =>
  (basisPoints / 100).toFixed(2);

const formatSsim = (raw: number | null): string =>
  raw === null ? 'N/A' : (raw / 10000).toFixed(4);

const statusBadge = (entry: ChangelogEntry): string => {
...
const renderRow = (entry: ChangelogEntry, showColumn: 'commit' | 'route'): string => {
  const firstCol =
    showColumn === 'commit'
...
<body><h1>Visual Changelog</h1><p class="empty">No visual changes recorded</p></body></html>`;
  ...
  const groups =
    groupBy === 'route' ? groupByRoute(entries) : groupByCommit(entries);

  const sections = Object.entries(groups)
    .map(([key, group]) => {
      const heading = escapeHtml(key);
      const showColumn: 'commit' | 'route' =
        groupBy === 'route' ? 'commit' : 'route';
