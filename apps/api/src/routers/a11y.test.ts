...
import { byRunIdHandler, byProjectHandler } from './a11y.js';
...
describe('a11yRouter', () => {
  ...
  describe('byRunId', () => {
    it('returns violations grouped by status with correct summary counts', async () => {
      ...
      const result = await byRunIdHandler(db as any, RUN_ID);
  ...
  describe('byProject', () => {
    it('returns total and new violation counts from most recent run', async () => {
      ...
      const result = await byProjectHandler(db as any, PROJ_ID);
